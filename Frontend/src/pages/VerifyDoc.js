...
import { useGenerateSignatureMutation, useUploadDocumentUrlMutation ,useVerifyDocumentMutation} from '../features/auth/authApiSlice';
...
function VerfyDoc() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if(pan.type!=='application/pdf' || aadhar.type!=='application/pdf'){
                setErrMsg('Upload only pdf')
                return
            }
            // if(pan.size>1048576 || aadhar.size>1048576){
            //     setErrMsg("size must be less than 1mb 😁")
            // }
            ...
            const res = await verifyDocument(data).unwrap()
            console.log(res)
            // setLoading(false)
    ...
    if (uploadLoading || generateLoading) return <Loading/>
